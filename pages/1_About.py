...
with col1:
    ...
    if auth.is_admin():
        ...
        new_comp_desc = st.text_area("Competition Description",
                                     value=about.get("competition_description", ""), height=150)
...
with col2:
    ...
    if auth.is_admin():
        new_team_desc = st.text_area("Team Description",
                                     value=about.get("team_description", ""), height=200)
    else:
    ...
    st.markdown(
        ...
        '<div style="font-family:\'Bebas Neue\',sans-serif;font-size:22px;color:#60A5FA;letter-spacing:3px">'
        'BRACU DUBURI</div>'
